...
param(
    [string] $SoftwareID,
    [switch] $DryRun,     # Validate everything but don't install
    [switch] $Force,      # Reinstall even if already present
    [switch] $List        # Show all available packages in catalog
)
...
# ── Require SoftwareID if not listing ────────────────────────────────────────
if (-not $SoftwareID) {
...
# ─────────────────────────────────────────────────────────────────────────────
# STEP 2 — Collect user & machine context
# ─────────────────────────────────────────────────────────────────────────────
Write-Log "" -LogFile $LogFile
Write-Log "STEP 2/8 — Collecting user & machine context..." -LogFile $LogFile
...
Write-Log "Running as Administrator ✓" -Level "SUCCESS" -LogFile $LogFile
...
try {
    ...
    if ($bitsAvailable) {
        ...
        Invoke-WebRequest -Uri $pkg.download_url -OutFile $installerPath `
...
# NOTE: In this lab version the catalog has placeholder hashes.
# In production, replace catalog.json sha256 values with real ones.
# To get the real hash after download: Get-FileHash .\installer.exe -Algorithm SHA256
...
if ($actualHash -ne $expectedHash) {
    ...
    # In production you would: exit 5
} else {
    Write-Log "Integrity verified ✓" -Level "SUCCESS" -LogFile $LogFile
}
